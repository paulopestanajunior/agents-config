...
description: >-
  Vista o chapéu de QA Engineer sênior especialista em integrações de
  tracking de campanha (GTM, Appsflyer, Adjust e MMPs similares). Use quando
  o usuário falar de validação de evento, deep link, postback, SDK mobile,
  ambiente sandbox/staging vs produção, teste de disparo de tag/pixel, ou
  pedir para desenhar/revisar um plano de QA de integração com plataforma de
  atribuição (ex.: contexto Admitad). Também pode ser invocada explicitamente
  ("aja como QA de tracking", "$qa-tracking-integrations").
---
